...
class CD_Wrapper(torch.nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, Px, Py):
        dl, dr = distChamferCUDA(Px, Py)

        return dl, dr

class EMD_Wrapper(torch.nn.Module):
    def __init__(self, accelerated_emd):
        super().__init__()
        self.accelerated_emd = accelerated_emd

    def forward(self, Px, Py):
        if self.accelerated_emd:
            emd_batch = emd_approx_cuda(Px, Py)
        else:
            emd_batch = emd_approx(Px, Py)
...
def _pairwise_EMD_CD_(sample_pcs, ref_pcs, batch_size, accelerated_cd=True,
                      accelerated_emd=True, use_data_parallel = True):
    ...
    if use_data_parallel:
        emd_module = EMD_Wrapper(accelerated_emd=accelerated_emd)
        emd_module = torch.nn.DataParallel(emd_module)
        cd_module = CD_Wrapper()
        cd_module = torch.nn.DataParallel(cd_module)
    
    num_sample = sample_pcs.shape[0]
    num_ref = ref_pcs.shape[0]
    total_num = num_sample * num_ref
    
    ##
    indices = [list(item) for item in product(range(num_sample), range(num_ref))]
    indices = torch.from_numpy(np.array(indices)).long().cuda()
    batch_num = int(np.ceil(total_num / batch_size))
    cd_results = []
    
    emd_results = []
    for i in tqdm(range(batch_num)):
        sampled_sample_pcs = sample_pcs[indices[i*batch_size:(i+1)*batch_size, 0]]
        sampled_ref_pcs = ref_pcs[indices[i*batch_size:(i+1)*batch_size, 1]]
        # import pdb;pdb.set_trace()
        
        # dl, dr = distChamferCUDA(sampled_sample_pcs, sampled_ref_pcs)
        if use_data_parallel:
            emd_res = emd_module(sampled_sample_pcs, sampled_ref_pcs)
            dl, dr = cd_module(sampled_sample_pcs, sampled_ref_pcs)
        else:
            emd_res = emd_approx_cuda(sampled_sample_pcs, sampled_ref_pcs)
            dl, dr = distChamferCUDA(sampled_sample_pcs, sampled_ref_pcs)
        
        emd_results.append(emd_res)
        cd_results.append(dl.mean(dim=1) + dr.mean(dim=1))


    reshaped_results = torch.cat(cd_results).view(num_sample, num_ref)
    emd_reshaped_results = torch.cat(emd_results).view(num_sample, num_ref)
    
    all_cd = reshaped_results
    all_emd = emd_reshaped_results
...
def compute_all_metrics(sample_pcs, ref_pcs, batch_size, accelerated_cd=False):
    ...
    # M_rs_cd = _pairwise_EMD_CD_(ref_pcs, sample_pcs, batch_size, accelerated_cd=accelerated_cd)
    # M_rs_emd = M_rs_cd
    ...
    # M_rr_cd = _pairwise_EMD_CD_(ref_pcs, ref_pcs, batch_size, accelerated_cd=accelerated_cd)
    # M_ss_cd = _pairwise_EMD_CD_(sample_pcs, sample_pcs, batch_size, accelerated_cd=accelerated_cd)
    # M_rr_emd = M_rr_cd
    # M_ss_emd = M_ss_cd
