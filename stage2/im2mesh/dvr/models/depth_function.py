...
class DepthModule(nn.Module):
    ...
    def forward(self, text, ray0, ray_direction, decoder, c=None, it=None,
                n_steps=None):
        ...
        #lzz add inputs 12
        if n_steps[1] > 1:
            inputs = [text, ray0, ray_direction, decoder, c, n_steps,
...
class DepthFunction(torch.autograd.Function):
    ...
    def run_Secant_method(f_low, f_high, d_low, d_high, n_secant_steps,
        ...
        for i in range(n_secant_steps):
            p_mid = ray0_masked + d_pred.unsqueeze(-1) * ray_direction_masked
            #print (p_mid.shape, c.shape,'mid c')
            with torch.no_grad():
                #print ('pmid', p_mid.shape,c.shape)

                #f_mid= decoder(p_mid.unsqueeze(0), c[0], text, pred_occ=1, pred_color=0)[0]-logit_tau 
                f_mid= decoder(p_mid, c[:,0,:,:,:], text, pred_occ=1, pred_color=0)[0]-logit_tau 
                '''for i in range(p_mid.shape[0]):
                  print (p_mid[i].unsqueeze(1).shape, 'f_mid',c[0].shape, flush=True)
                  f_m= decoder(p_mid[i].unsqueeze(1), c[0])[0]-logit_tau 
                  f_mid[i]=f_m'''
            ...
            #print (d_low.shape, d_pred.shape, ind_low.shape, 'd_low, dpred, indlow')
            #print (f_low.shape, f_mid.shape, ind_low.shape, 'f_low, fmid, indlow')
            if ind_low.sum() > 0:
    ...
    @staticmethod
    def perform_ray_marching(ray0, ray_direction, decoder, c,
                             tau=0.5, n_steps=[128, 129],text=None, n_secant_steps=8,
                             depth_range=[0., 2.4], method='secant',
        ...
        #print (p_split.shape, c.shape, decoder,'decoder')

        '''for p_split in torch.split(
            p_proposal.view(batch_size, -1, 3),
            int(max_points / batch_size), dim=1):
              #print ('1')
              #print (p_split.shape, c.shape)
              #print (decoder(p_split, c)[0].shape)'''
        
        
        
        #print (p_split.shape, c.shape, 'p_split, c')
        #print ('text',text)
        ...
        print (p_proposal.shape, 'p_proposal')

        for p_split in torch.split(p_proposal.view(batch_size, -1, 3),int(max_points / batch_size), dim=1):
          pred=decoder(p_split, c, text,  pred_occ=1, pred_color=0)[0] - logit_tau
          print (p_split.shape, pred.shape)
          if val==None:
            val=pred
          
          val = torch.cat((val,pred), dim=1)
        val=val.view(batch_size, -1, n_steps)
        print (val.shape, 'val')'''
        
        
        #print ('p_split', p_proposal.shape)
        #print (logit_tau, 'tat')
        #exit()
        ...
        with torch.no_grad():
            val = torch.cat([(
                #decoder(p_split, c, only_occupancy=True)
                decoder(p_split, c, text,  pred_occ=1, pred_color=0)[0] - logit_tau)
                for p_split in torch.split(
                    p_proposal.view(batch_size, -1, 3),
        ...
        #print (val.shape, 'val')
        #print (ray0.shape, c.shape, text.shape,'shape')        # Create mask for valid points where the first point is not occupied
        mask_0_not_occupied = val[:, :, 0] < 0
        #print (val.shape, 'val')
        # Calculate if sign change occurred and concat 1 (no sign change) in
        # last dimension
        
        #print ((val[:, :, :-1] * val[:, :, 1:]).shape, val.shape, 'val')
        #print ('n_pts', n_pts)
        ...
        #print (mask_sign_change.shape, 'mask_sign_change')
        ...
        #print (mask.shape,d_proposal.view(n, n_steps, 1).shape, d_proposal.view(n, n_steps, 1)[torch.arange(n), indices.view(n)].shape, d_proposal.view(n, n_steps, 1)[torch.arange(n), indices.view(n)].view( batch_size, n_pts).shape, d_proposal.shape, d_low.shape, val.shape, f_low.shape)
        ...
        if c is not None and c.shape[-1] != 0:
            c = c.unsqueeze(1)#.repeat(1, n_pts, 1,1,1)[mask]
        #print ('c.shape,',c.shape)
        # Apply surface depth refinement step (e.g. Secant method)
        
        #print (c.shape, f_low.shape,f_high.shape, d_low.shape, d_high.shape, ray0_masked.shape)
        if method == 'secant' and mask.sum() > 0:
            d_pred = DepthFunction.run_Secant_method(
                ...
                ray_direction_masked, decoder, c, logit_tau, text)
        ...
        #print ('d pred', d_pred_out.shape, pt_pred.shape, mask.shape, mask_0_not_occupied.shape)
    ...
    def forward(ctx, *input):
        ...
        #lzz input 12
        (text, ray0, ray_direction, decoder, c, n_steps,n_secant_steps, tau,
         depth_range, method, check_cube_intersection, max_points) = input[:12]
         
        #print ('text', text, 'tau', tau)

        # Get depth values
        with torch.no_grad():
            d_pred, p_pred, mask, mask_0_not_occupied = \
                DepthFunction.perform_ray_marching(
                    ray0, ray_direction, decoder, c, tau, n_steps, text,
    ...
    def backward(ctx, grad_output):
        ...
        #print (ray0.requires_grad, ray_direction.requires_grad, d_pred.requires_grad, p_pred.requires_grad, c.requires_grad,'backward requrie')
        decoder = ctx.decoder
        mask = ctx.mask
        eps = 1e-3
        ...
        with torch.enable_grad():
            ...
            #print ('backwrd', p_pred.shape, c.shape)
            f_p, _= decoder(p_pred, c,  pred_occ=1, pred_color=0)
            f_p_sum = f_p.sum()
        ...
        #print (grad_phi,' grad_phi',flush=True)
