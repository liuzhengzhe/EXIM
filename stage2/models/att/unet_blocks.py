...
from .resnet import Downsample3D, FirDownsample3D, FirUpsample3D, ResnetBlock3D, Upsample3D
...
class UNetMidBlock3DCrossAttn(nn.Module):
    ...
    def forward(self, hidden_states, temb=None, encoder_hidden_states=None, mask=None):
        hidden_states = self.resnets[0](hidden_states, temb)
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            hidden_states = attn(hidden_states, encoder_hidden_states, mask)
            hidden_states = resnet(hidden_states, temb)
...
class AttnSkipDownBlock3D(nn.Module):
    def __init__(
        ...
        if add_downsample:
            ...
            self.downsamplers = nn.ModuleList([FirDownsample3D(in_channels, out_channels=out_channels)])
            self.skip_conv = nn.Conv3d(3, out_channels, kernel_size=(1, 1), stride=(1, 1))
        else:
...
class CrossAttnUpBlock3D(nn.Module):
    ...
    def forward(self, hidden_states, res_hidden_states_tuple, temb=None, encoder_hidden_states=None, mask=None):
        for resnet, attn in zip(self.resnets, self.attentions):

            # pop res hidden states
            res_hidden_states = res_hidden_states_tuple[-1]
            res_hidden_states_tuple = res_hidden_states_tuple[:-1]
            #print (res_hidden_states_tuple, hidden_states.shape, res_hidden_states.shape)


            if res_hidden_states[-1].size(-1) < hidden_states.size(-1):
                hidden_states = hidden_states[..., :-1]
            if res_hidden_states[-1].size(-2) < hidden_states.size(-2):
                hidden_states = hidden_states[..., :-1, :]
            if res_hidden_states[-1].size(-3) < hidden_states.size(-3):
                hidden_states = hidden_states[..., :-1, :, :]
...
class AttnSkipUpBlock3D(nn.Module):
    def __init__(
        ...
        if add_upsample:
            ...
            self.skip_conv = nn.Conv3d(out_channels, 3, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
