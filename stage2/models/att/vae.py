...
from .unet_blocks import UNetMidBlock3D, get_down_block, get_up_block
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.conv_in = torch.nn.Conv3d(in_channels, block_out_channels[0], kernel_size=3, stride=1, padding=1)
...
class VQModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.quant_conv = torch.nn.Conv3d(latent_channels, latent_channels, 1)
        self.quantize = VectorQuantizer(
            num_vq_embeddings, latent_channels, beta=0.25, remap=None, sane_index_shape=False
        )
        self.post_quant_conv = torch.nn.Conv3d(latent_channels, latent_channels, 1)
...
class AutoencoderKL(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.quant_conv = torch.nn.Conv3d(2 * latent_channels, 2 * latent_channels, 1)
        self.post_quant_conv = torch.nn.Conv3d(latent_channels, latent_channels, 1)
