...
cfg = config.load_config('configs/single_view_reconstruction/multi_view_supervision/ours_combined.yaml', 'configs/default.yaml')
...
train_dataset_shape = voxelized_data.VoxelizedDataset('train', voxelized_pointcloud= args.pointcloud, pointcloud_samples= args.pc_samples, res=args.res, sample_distribution=args.sample_distribution,
                                          sample_sigmas=args.sample_sigmas ,num_sample_points=50000, batch_size=args.batch_size, num_workers=30)

val_dataset_shape = voxelized_data.VoxelizedDataset('val', voxelized_pointcloud= args.pointcloud , pointcloud_samples= args.pc_samples, res=args.res, sample_distribution=args.sample_distribution,
                                          sample_sigmas=args.sample_sigmas ,num_sample_points=50000, batch_size=args.batch_size, num_workers=30)
...
trainer = training.Trainer(net,torch.device("cuda"),train_dataset, val_dataset,train_loader,val_loader, train_dataset_shape, val_dataset_shape, exp_name,cfg, optimizer=args.optimizer)
trainer.train_model()