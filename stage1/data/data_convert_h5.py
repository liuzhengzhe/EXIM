...
if __name__ == '__main__':
    save_folder = r'/research/dept6/khhui/sdf_samples_scaled/04379243'
    workers = 6
    resolution_index = config.max_depth
    clip_value = 0.1
    save_new_path =  os.path.join(r'/research/dept6/khhui/sdf_samples_scaled', os.path.basename(save_folder) + f'_{clip_value if clip_value is not None else "no_clip"}_{config.wavelet_type}_{resolution_index}_{padding_mode}.npy')

    paths = [ os.path.join(save_folder, file) for file in os.listdir(save_folder) if file.endswith('.npy')]
