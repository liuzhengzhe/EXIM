...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        '''#print (mask.shape, mode.shape, mode, 'mask mode',flush=True)
        #exit()
        ...
        mask2=torch.zeros(text.shape).to(device=sample.device)
        mask2[:,0]=1 
        mask2=mask2*mode
        
        
        #print (mask, mask2, 'mask, mask2')
        ...
        #print (encoder_hidden_states2.shape,clip_feature.shape, flush=True)

        encoder_hidden_states2[:,0:1,:]=clip_feature
        ...
        #print (encoder_hidden_states, mode, encoder_hidden_states2)
        #print (encoder_hidden_states.shape, mode.shape, encoder_hidden_states2.shape)
        
        
        
        mode=mode.unsqueeze(-1)
        encoder_hidden_states=encoder_hidden_states*(1-mode)+ encoder_hidden_states2*mode'''
