...
class FirUpsample3D(nn.Module):
    ...
    def _upsample_3d(self, x, weight=None, kernel=None, factor=2, gain=1):
        ...
        if self.use_conv:
            ...
            x = F.conv_transpose3d(x, weight, stride=stride, output_padding=output_padding, padding=0)

            x = upfirdn3d_native(x, torch.tensor(kernel, device=x.device), pad=((p + 1) // 2 + factor - 1, p // 2 + 1))
        else:
    ...
    def forward(self, x):
        if self.use_conv:
            height = self._upsample_3d(x, self.Conv3d_0.weight, kernel=self.fir_kernel)
            height = height + self.Conv3d_0.bias.reshape(1, -1, 1, 1)
        else:
            height = self._upsample_3d(x, kernel=self.fir_kernel, factor=2)
...
class FirDownsample3D(nn.Module):
    ...
    def _downsample_3d(self, x, weight=None, kernel=None, factor=2, gain=1):
        ...
        if self.use_conv:
            ...
            p = (kernel.shape[0] - factor) + (convW - 1)
            s = [factor, factor]
            x = upfirdn3d_native(x, torch.tensor(kernel, device=x.device), pad=((p + 1) // 2, p // 2))
            x = F.conv3d(x, weight, stride=s, padding=0)
        else:
            p = kernel.shape[0] - factor
            x = upfirdn3d_native(x, torch.tensor(kernel, device=x.device), down=factor, pad=((p + 1) // 2, p // 2))

        return x

    def forward(self, x):
        if self.use_conv:
            x = self._downsample_3d(x, weight=self.Conv3d_0.weight, kernel=self.fir_kernel)
            x = x + self.Conv3d_0.bias.reshape(1, -1, 1, 1)
        else:
            x = self._downsample_3d(x, kernel=self.fir_kernel, factor=2)
...
def upsample_3d(x, kernel=None, factor=2, gain=1):
...
def downsample_3d(x, kernel=None, factor=2, gain=1):
    ...
    kernel = kernel * gain
    p = kernel.shape[0] - factor
    return upfirdn3d_native(x, torch.tensor(kernel, device=x.device), down=factor, pad=((p + 1) // 2, p // 2))


def upfirdn3d_native(input, kernel, up=1, down=1, pad=(0, 0)):
    up_x = up_y = up
    down_x = down_y = down
    pad_x0 = pad_y0 = pad[0]
    pad_x1 = pad_y1 = pad[1]
