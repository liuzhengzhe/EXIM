...
testing_folder='../data/model/table/'
...
def one_generation_process(args):
    ...
    with torch.no_grad():
        ...
        dic=np.load('../data/official_table_test.npy',allow_pickle=True)[()]
        ...
        for key in dic.keys():
          ...
          for text_str in dic[key]:
           ...
           for repeat in range(1):
            if os.path.exists('mesh/'+key+'_'+text_str[:40].replace('/','')+'_'+str(repeat)+'high.obj'):
              print ('pass')
              continue
            testing_sample_index = testing_indices[m]
            #print (testing_sample_index, 'testing')
            ...
            #text_str='a small simple chair '+text_str
            ...
            if not evaluate_gt:
                ...
                #print (highs_samples[0].shape,highs_samples[1].shape,highs_samples[2].shape,'high samples')
                #exit()
                      
                np.save('feat_evaluation/'+key+'_'+text_str[:20].replace('/','')+'_'+str(repeat)+'low.npy',low_samples.detach().cpu().numpy())
