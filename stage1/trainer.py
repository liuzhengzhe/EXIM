...
class Trainer(object):
    ...
    def train_network(self):
        ...
        for idx in range(self.config.starting_epoch, self.config.training_epochs + 1):
            ...
            with tqdm(data_loader, unit='batch') as tepoch:
                ...
                network.train()
                ## main training loop
                for data,  text, code_indices, clip_feature, mode in tepoch:
                    ## remove gradient
                    optimizer.zero_grad()
                    ...
                    if hasattr(self.config, 'mix_precision') and self.config.mix_precision:
                        with autocast():
                            ...
                            mse_loss = self.gt_supervision(diffusion_module, highs_lap, low_lap, text,clip_feature,mode,
    ...
    def gt_supervision(self, diffusion_module, highs_lap, low_lap, feature,clip_feature,mode, mse_fuction, mse_loss, network,
        ...
        for j in range(self.config.max_depth + 1):
            ...
            if j == self.config.max_depth:
                ...
                iterative_loss = diffusion_module.training_losses(model=network, x_start=low_lap, t=t, text=feature, clip_feature=clip_feature, mode=mode)
...
if __name__ == '__main__':
    ...
    debugger = MyDebugger(f'Wavelet-Training-experiment{"-" + config.special_symbol if len(config.special_symbol) > 0 else config.special_symbol}', is_save_print_to_file = False, config_path = resume_path)
