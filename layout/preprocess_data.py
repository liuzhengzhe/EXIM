...
def main(argv):
    ...
    #print (dataset.class_labels, dataset.object_types, dataset.count_furniture)

    
    
    
    
    category_list=dataset.class_labels
    ...
    dataset = ThreedFront.from_dataset_directory(
        ...
        filter_fn=filter_function(
             config, ["test"], args.without_lamps  #config, ["train", "val", "test"], args.without_lamps
        )
    )
    ...
    category_dic=np.load('category_dic.npy',allow_pickle=True)[()]

    f_wrong=open('wrong.txt', 'w')
    
    test_f=open(args.dataset_filtering+'test_file.txt', 'w')
    ...
    for (i, es), ss in tqdm(zip(enumerate(encoded_dataset), dataset)):
      try:
        ...
        with DirLock(room_directory + ".lock") as lock:
            ...
            floor_plan, _, _,floor_image, offset_scale = floor_plan_from_scene(
                ss, args.path_to_floor_plan_textures, without_room_mask=True
            )
            


            paths=get_textured_objects_in_scene(
                ss, es["class_labels"], offset_scale,dataset.class_labels, category_dic, category_list,ignore_lamps=args.without_lamps,  
            )
            ...
            renderables, object_images, object_count = get_textured_objects_in_scene(
                ss, es["class_labels"], offset_scale,dataset.class_labels, category_dic, category_list,ignore_lamps=args.without_lamps,  
            )
            

            
            cv2.imwrite(room_directory+'/floor.png',floor_image)
            cv2.imwrite(room_directory+'/semantic.png',object_images[0])      
            cv2.imwrite(room_directory+'/height.png',object_images[1])    
            cv2.imwrite(room_directory+'/height_low.png',object_images[2])    
            cv2.imwrite(room_directory+'/orientation.png',object_images[3])    
            cv2.imwrite(room_directory+'/instance.png',object_images[4])
            cv2.imwrite(room_directory+'/instance_lamp.png',object_images[5])
            np.save(room_directory+'/feature.npy',object_images[6])
            np.save(room_directory+'/feature_lamp.npy',object_images[7])
            
            f=open(room_directory+'/text','w')
            f.write('a '+args.dataset_filtering.split('_')[-1]+' with ')
            f.write(str(object_count[0])+' cabinet, ')
            f.write(str(object_count[1])+' bed, ')
            f.write(str(object_count[2])+' chair, ')
            f.write(str(object_count[3])+' table, ')
            f.write(str(object_count[4])+' sofa, and ')
            f.write(str(object_count[5])+' lighting. ')
            f.close()
        ...
        f_wrong.write(room_directory)
        #os.system('rm -r '+room_directory)
      f_wrong.close()
