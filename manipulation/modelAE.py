...
class generator(nn.Module):
	def __init__(self, z_dim, point_dim, gf_dim):
		...
		self.gf_dim = gf_dim
		self.linear_1 = nn.Linear(128+self.point_dim, 1024, bias=True)
		self.linear_2 = nn.Linear(1024, 256, bias=True)
		self.linear_3 = nn.Linear(256, 4, bias=True)
...
class encoder(nn.Module):
	def __init__(self, ef_dim, z_dim):
		...
		self.ef_dim = ef_dim
		self.z_dim = z_dim
		self.conv_1 = nn.Conv3d(1, self.ef_dim, 4, stride=2, padding=1, padding_mode='zeros', bias=True)
		self.in_1 = nn.InstanceNorm3d(self.ef_dim, affine=True)
		self.conv_2 = nn.Conv3d(self.ef_dim, self.ef_dim*2, 4, stride=2, padding=1, padding_mode='zeros',bias=True)
		self.in_2 = nn.InstanceNorm3d(self.ef_dim*2,affine=True)
		self.conv_3 = nn.Conv3d(self.ef_dim*2, self.ef_dim*4, 4, stride=2, padding=1,padding_mode='zeros', bias=True)
		self.in_3 = nn.InstanceNorm3d(self.ef_dim*4, affine=True)
		self.conv_4 = nn.Conv3d(self.ef_dim*4, self.ef_dim*8, 4, stride=2, padding=1, padding_mode='zeros',bias=True)
		self.in_4 = nn.InstanceNorm3d(self.ef_dim*8,affine=True)
		self.conv_5 = nn.Conv3d(self.ef_dim*8, 128, 4, stride=1, padding=0, bias=True)
	...
	def forward(self, inputs, is_training=False):
		#print ('inputs', inputs, torch.unique(inputs))
		d_1 = self.conv_1(inputs)
		#print ('d1',torch.permute(d_1, (0,2,3,4,1)), d_1.shape, d_1[:,0,:,:,:])
		d_1=self.in_1(d_1)
		#print ('d2',torch.permute(d_1, (0,2,3,4,1)), d_1.shape, torch.unique(d_1))
		d_1 = F.leaky_relu(d_1, negative_slope=0.02, inplace=True)

		d_2x = self.conv_2(d_1)
		d_2=self.in_2(d_2x)
		...
		#print (d_1.shape, d_2.shape, d_3.shape, d_4.shape, d_5.shape)
...
class IM_AE(object):
	def __init__(self):
		...
		#2-- (32, 16*16*16)
		#3-- (64, 16*16*16*4)
		self.sample_vox_size = 64 #config.sample_vox_size
		...
		data_hdf5_name = '/mnt/sda/lzz/BSP-NET-pytorch/data/data_per_category/03001627_chair/03001627_vox256_img_train.hdf5' #self.data_dir+'/'+self.dataset_load+'.hdf5'
		...
		self.sampling_threshold = 0.4 #final marching cubes threshold
	...
	def train(self, config):
		...
		if os.path.exists(checkpoint_txt):
			...
			print(" [!] Load failed...")
		torch.save(self.im_network.state_dict(), 'save.pth')
		exit()
	...
	def z2voxel(self, z):
		model_float = torch.zeros([self.real_size+2,self.real_size+2,self.real_size+2]).cuda()
		dimc = self.cell_grid_size
		dimf = self.frame_grid_size
		
		frame_flag = torch.zeros([dimf+2,dimf+2,dimf+2]).cuda().long()
		queue = []
		...
		for i in range(frame_batch_num):
			...
			#model_out=model_out[:,category]
			result=torch.argmax(model_out,1)
			frame_flag[x_coords+1,y_coords+1,z_coords+1] = torch.reshape(result.long(), [self.test_point_batch_size])
		...
		while len(queue)>0:
			...
			model_out_batch = model_out_batch_.detach().cpu().numpy()[0]#[:,category]
			print ('model_out_batch', model_out_batch.shape)
			for i in range(batch_num):
				point = point_list[i]
				model_out = torch.argmax(model_out_batch[i*cell_batch_size:(i+1)*cell_batch_size],1)
	...
	def test_mesh_point(self, config):
		...
		if 1: #os.path.exists(checkpoint_txt):
			#fin = open(checkpoint_txt)
			model_dir = 'init.pth' #fin.readline().strip()
			#fin.close()
		...
		for t in range(config.start, min(len(self.data_voxels),config.end)):
			...
			data=np.load('/mnt/sda/lzz/BAE-NET/data.npy')
			#print (data.shape, np.unique(data))

			batch_voxels[0,0,:,:,:]=torch.from_numpy(data).cuda()[0,:,:,:,0]
			#print ('batch_voxels',batch_voxels.shape, torch.unique(batch_voxels))      
			model_z,_ = self.im_network(batch_voxels, None, None, is_training=False)

			if 1: 
			  model_float = self.z2voxel(model_z)
			  print (model_float.shape, 'model_float')
			  from plyfile import PlyData,PlyElement
			  some_array=[]
			  for i in range(1,65):
			   for j in range(1,65):
			    for k in range(1,65):
			     if batch_voxels[int(i-1),int(j-1),int(k-1)]>0.5:
			      some_array.append((i,j,k,model_float[2,int(i),int(j),int(k)]*50,model_float[1,int(i),int(j),int(k)]*50,model_float[0,int(i),int(j),int(k)]*50))
			  ...
			  PlyData([el]).write('part_seg.ply')

			  '''vertices, triangles = mcubes.marching_cubes(model_float, self.sampling_threshold)
			  vertices = (vertices.astype(np.float32)-0.5)/self.real_size-0.5
			  write_ply_triangle(config.sample_dir+"/"+str(t)+"_"+str(category)+"_vox.ply", vertices, triangles)

			  sampled_points_normals = sample_points_triangle(vertices, triangles, 4096)
			  np.random.shuffle(sampled_points_normals)
			  write_ply_point_normal(config.sample_dir+"/"+str(t)+"_"+str(category)+"_pc.ply", sampled_points_normals)'''
			exit()
