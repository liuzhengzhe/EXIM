...
\
                ...
                t2=torch.zeros((1)).cuda().long()

                diffss=torch.zeros((data3.shape)).cuda()
                for i in range(0,100,10):
                  for repeat in range(1):
                    ...
                    x_t=diffusion_module_train.q_sample(data3, t)
                    x_t2=diffusion_module_train.q_sample(data3, t2)
                    ...
                    mask=x_t.clone()
                    mask[:]=0
                    #x_t_mask=torch.cat((x_t,mask,data3),1)


                    model_kwargs = {'noise_save_path': os.path.join(folder_path, f'{m+start_index}_{testing_sample_index}_noise.pt'), 'text': text, 'start_step':100-i,  'end_step': 100-i}
                    #model_kwargs_mani = { 'text': text_mani,  'inpaint_image': data3, 'inpaint_mask': mask}
                    ...
                    #print (x_t.shape, t.shape, data3.shape)
                    model_outputs1=diffusion_module_train._predict_eps_from_xstart(x_t2, t2, data3).unsqueeze(0)
                    #model_outputs1=model_outputs1#.unsqueeze(0)


                    setup_seed(i)
                    model_kwargs_mani = {'noise_save_path': os.path.join(folder_path, f'{m+start_index}_{testing_sample_index}_noise.pt'), 'text': text_mani, 'start_step':  100-i,  'end_step': 100-i}
                    low_samples1, model_outputs1 = diffusion_module.ddim_sample_loop(model=network,
                    ...
                    low_samples1=low_samples1.detach()
                    ...
                    model_kwargs_mani = {'noise_save_path': os.path.join(folder_path, f'{m+start_index}_{testing_sample_index}_noise.pt'), 'text': text_mani, 'start_step':  100-i,  'end_step': 100-i}
                    low_samples2, model_outputs2 = diffusion_module.ddim_sample_loop(model=network,
                                                                        shape=[1, 1] + dwt_sparse_composer.shape_list[-1],
                                                                        noise=x_t,
                                                                        device=device,
                                                                        clip_denoised=clip_noise, progress=True,
                    ...
                    low_samples2=low_samples2.detach()

                    
                    
                    diff=torch.zeros((model_outputs1[0].shape)).cuda()
                    for j in range(len(model_outputs1)):
                      diff+=abs(model_outputs1[j]-model_outputs2[j])
                ...
                PlyData([el]).write('seg_predition.ply')
                ...
                for cate in range(4):
                  ...
                  whe=torch.where((model_float.unsqueeze(0).unsqueeze(0)==cate) & (batch_voxels_down_46==1))
                  k=whe[0].shape[0] #min(100, whe[0].shape[0])
                  topk=torch.topk(diffss_scale[whe], k)[0] 
                  avg=torch.mean(topk) 
                  print (cate, avg,'avg')
                  if avg>avg_max:
                    maxi=cate
                    avg_max=avg
                  diffss_scale[whe]=avg  
                  
                #maxi=0
                #maxi2=1
                print ('the part to manipulate..', maxi)


                #exit(0)
                
                
                batch_voxels_down_46_dilate=dilate(batch_voxels_down_46,3)
                diffss_scale[torch.where(((model_float.unsqueeze(0).unsqueeze(0)!=maxi) | (model_float.unsqueeze(0).unsqueeze(0)==-1)) & (batch_voxels_down_46_dilate==1))]=0                
                diffss_scale[torch.where(((model_float.unsqueeze(0).unsqueeze(0)==maxi)) & (batch_voxels_down_46==1))]=1

                diffss_scale[torch.where(batch_voxels_down_46==0)]=0

                #diffss_scale[torch.where(batch_voxels_down_46==1)]=1             
                
                #diffss_scale[torch.where(((model_float.unsqueeze(0).unsqueeze(0)<=0.7) | (model_float.unsqueeze(0).unsqueeze(0)==-1)) & (batch_voxels_down_46_dilate==1))]=0    
                #diffss_scale[torch.where((diffss_scale>0.7) & (batch_voxels_down_46==1))]=1
                ...
                for i in range(0,46):
                    for j in range(0,46):
                        for k in range(0,46):
                            if diffss_scale[0,0,i,j,k]>0.5:
                                some_array.append((i/46-0.5,j/46-0.5,k/46-0.5,diffss_scale[0,0,i,j,k]*255,diffss_scale[0,0,i,j,k]*255,diffss_scale[0,0,i,j,k]*255))
                some_array = np.array(some_array, dtype=[('x', 'float32'), ('y', 'float32'),    ('z', 'float32'),   ('red', 'uint8'),    ('green', 'uint8'),    ('blue', 'uint8')])
                el = PlyElement.describe(some_array, 'vertex')
                PlyData([el]).write('init2_after_seg.ply')
                ...
                mask[:]=1
                whe=torch.where(diffss_scale>0.5)
                #print (whe)
                #mask[whe]=0
                #mask=dilate(mask,3)
                #whe=torch.where(mask==0)
                #print (whe)
...
'
                ...
                mask[torch.where((data3<=0) & (model_float.unsqueeze(0).unsqueeze(0)!=maxi))]=1
