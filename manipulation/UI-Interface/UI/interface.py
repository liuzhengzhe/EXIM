...
class MainInterface(QMainWindow):
    ...
    def initUI(self):
        ...
        if self.use_point_gen:
            ...
            self.main_opengl_widget = OpenGLWidget(use_part_seg=self.use_part_seg, points = points[0], initial_fov = self.INITIAL_FOV, ball_size =self.INITIAL_BALL_SIZE / self.BALL_RATIO)
            update_cnt = 1
            if self.use_two_screen:
                self.another_opengl_widget = OpenGLWidget(use_part_seg=False, points = points[1], initial_fov = self.INITIAL_FOV, ball_size =self.INITIAL_BALL_SIZE / self.BALL_RATIO)
            ...
            self.main_opengl_widget = OpenGLWidget(use_part_seg=self.use_part_seg, initial_fov = self.INITIAL_FOV, ball_size =self.INITIAL_BALL_SIZE / self.BALL_RATIO)
            if self.use_two_screen:
                self.another_opengl_widget = OpenGLWidget(use_part_seg=False, initial_fov = self.INITIAL_FOV, ball_size =self.INITIAL_BALL_SIZE / self.BALL_RATIO)
            self.pointcloud_indices[0] = self.pointcloud_cnt
    ...
    def init_sidebar_functions(self):
        ...
        button = QPushButton("Load Noise")
        button.clicked.connect(self.load_noise)
        self.sidebar_main_subwidgets.append(button)
        
        ## load selection
        button = QPushButton("Load Selection")
        button.clicked.connect(self.load_selection)
        self.sidebar_main_subwidgets.append(button)

        ## save selection
        button = QPushButton("Save Selection")
        button.clicked.connect(self.save_selection)
        self.sidebar_main_subwidgets.append(button)
    ...
    def interpolate_pointclouds(self):
        ...
        if self.direction_checkbox.isChecked():
            new_points = self.point_gen.interpolation_noise(z1 = self.noises[0], z2 = self.noises[1], selection = selection, alpha = self.alpha, use_latent = use_latent)
            ...
            new_points = self.point_gen.interpolation_noise(z1 = self.noises[1], z2 = self.noises[0], selection = selection, alpha = self.alpha, use_latent = use_latent)
            MainInterface.update_pointcloud(self.another_opengl_widget, new_points[0])
            self.save_interpolation_points(self.alpha, new_points[0])

        np.save(os.path.join(self.debugger.file_path(self.get_save_folder_name()), f'noise_{self.interpolation_cnt}_{self.alpha}.npy'), self.noises[0] * (1 - self.alpha) + self.noises[1] * self.alpha)

        direction_index= 0 if self.direction_checkbox.isChecked() else 1
        self.current_noise = np.copy(self.noises[direction_index])
        self.current_noise[selection == 1, :] = self.current_noise[selection == 1, :] * (1 - self.alpha) + self.noises[1 - direction_index, selection == 1] * self.alpha
    ...
    def save_current_selection_noise(self):
        ...
        if np.any(selection_flag):
            ...
            ## save the noise
            np.save(self.debugger.file_path(f'model_{self.pointcloud_indices[direction_index]}_partial_{self.partial_noise_index}_noise.npy'), retrieved_noise)
            np.save(self.debugger.file_path(f'model_{self.pointcloud_indices[direction_index]}_partial_{self.partial_noise_index}_selection.npy'), selection)
    ...
    def save_basic_points_info(self):
        ...
        ## save noise
        np.save(os.path.join(folder_path, f'noise_{self.pointcloud_indices[0]}.npy'), self.noises[0])
        if self.use_two_screen:
            np.save(os.path.join(folder_path, f'noise_{self.pointcloud_indices[1]}.npy'), self.noises[1])

        ## save the xyz files
        points_0 = pc_normalize(self.points[0])

        np.savetxt(os.path.join(folder_path, f'model_{self.pointcloud_indices[0]}.xyz'), points_0)
        if self.use_two_screen:
            points_1 = pc_normalize(self.points[1])
            np.savetxt(os.path.join(folder_path, f'model_{self.pointcloud_indices[1]}.xyz'), points_1)
    ...
    def save_interpolation_points(self, alpha, points):
        ...
        np.savetxt(os.path.join(folder_path, f'interpolation_{text_out}_{self.interpolation_cnt}_{alpha}.xyz'), points_output)
        np.save(os.path.join(folder_path, f'selection_{text_out}_{self.interpolation_cnt}_{alpha}.npy'), self.main_opengl_widget.point_selector.pointcloud_selected)

        self.interpolation_cnt += 1


    def get_save_folder_name(self):
        indices_in_str = [str(index) for index in self.pointcloud_indices]
        return "_".join(indices_in_str)

    def load_noise(self):
        options = QFileDialog.Options()
        options |= QFileDialog.DontUseNativeDialog

        fileName, _ = QFileDialog.getOpenFileName(self, "Open a Noise Vector", directory = self.debugger.file_path('.'), initialFilter= "npy file (*.npy)", options = options)
        if len(fileName) > 0 and fileName is not None:
            print(f"Open : {fileName}")
            try:
                if self.load_checkbox.isChecked():
                    self.noises[0] = np.load(fileName)
    ...
    def load_selection(self):
        ...
        fileName, _ = QFileDialog.getOpenFileName(self, "Open a Selection File", directory = self.debugger.file_path('.'), initialFilter= "npy file (*.npy)", options = options)
        if len(fileName) > 0 and fileName is not None:
            print(f"Open : {fileName}")
            try:
    ...
    def save_selection(self):
        selection = self.main_opengl_widget.point_selector.pointcloud_selected
        np.save(self.debugger.file_path(f'selection_{self.selection_cnt}.npy'), selection)
    ...
    def load_noise_on_selection(self):
        ...
        fileName, _ = QFileDialog.getOpenFileName(self, "Open a Partial Noise File", directory = self.debugger.file_path('.'), initialFilter= "npy file (*.npy)", options = options)
        if len(fileName) > 0 and fileName is not None:
            print(f"Open : {fileName}")
            try:
    ...
    def update_noise_search_result(self):
        ...
        new_noise[selections == 1] = self.current_noise_dir[np.newaxis, :] * self.current_noise_dir_ratio + new_noise[selections == 1]
    ...
    def change_model_dir(self, value):
        ...
        selections = self.main_opengl_widget.point_selector.pointcloud_selected if self.direction_checkbox.isChecked() else self.another_opengl_widget.point_selector.pointcloud_selected
        new_noise = np.copy(self.noises[change_index])
        new_noise[selections == 1] = self.current_noise_dir[np.newaxis, :] * (value / self.MAX_SCALE_NOISE) * self.MAX_NOISE_RATIO + new_noise[selections == 1]
        new_points = self.point_gen.generate_points_from_noise(new_noise)
        if change_index == 0:
            MainInterface.update_pointcloud(self.main_opengl_widget, new_points[0])
        else:
...
if __name__ == '__main__':
    debugger = MyDebugger("Main_Interface")
