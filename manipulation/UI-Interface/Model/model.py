...
from Model.point_operation import normalize_point_cloud
from collections import namedtuple
from Model.point_operation import plot_pcd_multi_rows,plot_pcd_multi_rows_single_color,plot_pcd_three_views_color,plot_pcd_multi_rows_color
...
root = os.getcwd()[:5]
plot_folder = root + "/lirh/pointcloud2/PointGeneration/experiments/plots/plot_mask_inte"
plot_folder = "plots"
...
class Model(object):
    ...
    def noise_generator(self, bs=1, masks=None):
        ...
        sim_noise = Variable(torch.Tensor(noise)).cpu()
    ...
    def sphere_generator(self, bs=2, static=True):
        ...
        ball = Variable(torch.Tensor(ball)).cpu()

        return ball

    def read_ball(self,sort=False):
        x = np.loadtxt(self.ball_path)
        ball = pc_normalize(x)
    ...
    def draw_correspondense(self):
        ...
        for i in range(sample_num):
            ...
            with torch.no_grad():
                z = Variable(torch.Tensor(noise)).cpu()
                out_pc = self.G(x, z)
                out_pc = out_pc.transpose(2,1)
